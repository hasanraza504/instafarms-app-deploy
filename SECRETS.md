...
|---|---|
| `SOURCE_REPO` | `InstaFarms/Instafarms-app` |
...
| Name | What it is |
|---|---|
| `SOURCE_REPO_SSH_KEY` | Private half of a **read-only deploy key** on `InstaFarms/Instafarms-app` |
...
on `InstaFarms/Instafarms-app` with **write access disabled**. This is deliberately preferred
...
Revoke or rotate it under `InstaFarms/Instafarms-app` → Settings → Deploy keys.
